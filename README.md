...
個人の認知構造を Git リポジトリで管理するための、構造化された OS です。LLM に依存せず、フォーク可能。

### 問題

思考が Obsidian・チャット履歴・AI のシステムプロンプトに分散し、それぞれ別のフォーマットで、別のプラットフォームにロックされている。AI 企業はあなたのコンテキストをサーバーに蓄積しようとしている。知られれば知られるほど、移行が難しくなる。

### 考え方

認知構造が、自分所有の Git リポジトリに住んでいたら？

- どの LLM でも読める。モデルをいつでも切り替えられる。コンテキストはそのまま残る。
- 層構造になっている。事実・解釈・決断・仮説が別々の層に分かれて混在しない。
- 層に整理するという行為が、思考を強制的に明確にする。単なる保存ではなく、思考インフラだ。
...
git clone https://github.com/getty77/cognitive-commons.git my-os
cd my-os
# まず logs/ から始める——今日起きたことをそのまま書く
# 熟成したものを上の層へ移動させる
```

リポジトリ（または必要な層）を任意の LLM にコンテキストとして渡す。これがあなたの `.claude`、あなたの `.chatgpt` ——モデル非依存のまま。

### なぜ "Commons" か

認知インフラはコモンズであるべきだ。プラットフォームに所有されず、サブスクリプションの壁の後ろにない。テンプレートは無料。あなたのインスタンスはあなたのもの。
